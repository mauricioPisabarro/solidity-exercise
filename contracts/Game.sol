...
contract Game is IGame {
    ...
    modifier onlyIfBossExists(uint256 _bossId) {
        require(_bossId < _bosses.length && _bossId >= 0, "Boss does not exist");
        _;
    }

    modifier onlyIfBossIsPopulated() {
        require(_currentBoss >= 0 && uint256(_currentBoss) < _bosses.length, "Current boss not populated");
    ...
    modifier onlyOwningExistingLivingCharacterWithXP() {
        require(
            _userHasCharacter[msg.sender],
            "You need to own a character"
        );
        Character character = _userCharacterMap[msg.sender];
        require(
            character.isDead() == false,
            "You need a living character"
        );
        require(
    ...
    function createBoss(uint256 _hps, uint256 _reward, uint256 _damage) external onlyAdmin returns (uint256) {
    ...
    function createCharacter() external onlyOneCharacterPerAddress returns (uint256) {
    ...
    function attackBoss() external onlyIfBossIsPopulated needToOwnACharacter returns (bool) {
        Character character = _userCharacterMap[msg.sender];
        require(
            !character.isDead(),
            "Character is dead, can't attack"
        );
        Boss boss = _bosses[uint256(_currentBoss)];

        bool isBossDefeated = boss.receiveAttack(character.getAttackDamage());
        if (isBossDefeated) {
            character.receiveExperiencePoints(boss.getDefeatedReward());
            _defeatedBosses[_currentBoss] = true;
            _currentBoss = -1;
            return true;
        }

        bool isCharacterDefeated = character.receiveAttack(boss.getAttackDamage());
    ...
    function heal(uint256 _characterId) onlyOwningExistingLivingCharacterWithXP characterExists(_characterId) external {
